 ...
 * @param {String} localDist 本地Dist路径
 * @param {String} targetDistName 目标Dist文件夹名称
 * @return {String} 压缩后的zip文件路径
 */
async function compress(localDist, targetDistName) {
    let spinner = ora(chalk.rgb(255, 150, 0)("压缩中...")).start();
    try {
        let zipFilePath = resolvePath("./", localDist);
        let file = new AdmZip();
        file.addLocalFolder(zipFilePath, targetDistName);
 ...
 * @param {String} targetZipPath 目标zip路径
 * @param {String} targetDistDir 目标dist上级路径
 * @return {*} 
 */
async function decompress(targetZipPath, targetDistDir) {
    let spinner = ora(chalk.rgb(255, 150, 0)("解压中...")).start();
    try {
        // 修改压缩文件权限
        await sshCommand(`chmod 777 ${targetZipPath}`);
        // 解压缩文件
        await sshCommand(`unzip -o ${targetZipPath}`, targetDistDir);
        // 清除压缩文件
...
async function deploy(_CONFIG) {
    ...
    // 压缩本地打包项目打包文件 返回本地zip路径
    let localZipPath = await compress(localDist.handled, targetDist.basename);
    ...
    // 远程zip路径
    let targetZipPath = targetDist.dirname + "/" + handlePath(localZipPath).basename;

    // dist压缩文件上传
    await upload(localZipPath, targetZipPath);

    // 解压服务器中的dist压缩文件
    await decompress(targetZipPath, targetDist.dirname);
