...
async function compressDist(localDist, distZipName) {
    let targetPath = resolvePath("./", localDist.basename);
    let file = new AdmZip();
    file.addLocalFolder(targetPath, "dist");
    file.writeZip(distZipName);
    return { distZipPath: resolvePath("./", distZipName), distZipName }
}
...
async function upload(localDist, targetDist) {
    console.log(localDist, targetDist, '')
    let spinner = ora(chalk.rgb(255, 150, 0)("文件上传中...")).start();
    await conn.putFile(localDist, targetDist).then(() => {
...
async function deploy(_CONFIG) {
    ...
    // 压缩本地打包项目打包文件
    let { distZipPath, distZipName } = await compressDist(handlePath(localDist), "dist.zip");
    ...
    if (targetDistIsExist) {
        ...
        if (!override) {
            replaceDist(targetDist);
        }
    }

    // 打包文件上传
    await upload(distZipPath, targetDist + "/" + distZipName);
